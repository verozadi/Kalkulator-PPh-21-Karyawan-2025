...
const Reports: React.FC<ReportsProps> = ({ employees, masterEmployees, profile }) => {
  ...
  const handleExportXML = () => {
    ...
    if (filteredEmployees.length === 0) {
        alert(`Tidak ada data PPh 21 untuk periode ${selectedMonth}/${selectedYear}.`);
        return;
    }
...
${dataRows}
  ...
  const uniqueYears = useMemo(() => Array.from(new Set(employees.map(e => e.periodYear))).sort((a,b) => b - a), [employees]);
